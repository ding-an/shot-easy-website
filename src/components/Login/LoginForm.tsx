...
export async function register({
  ...
  const { data } = await post(
    `/register?${qs.stringify({ utm_source, cp_id })}`,
    {
      code,
      email,
    ...
    <form onSubmit={onSubmit} className="py-4">
      ...
      <div className={`relative mb-4`}>
        <input
          ...
          className={`h-12 w-full rounded-full bg-blue-100 px-4 text-base text-black outline-none placeholder:text-white-48 focus-visible:border ${
            ...
            authTips
              ? "border-blue-600 outline outline-1 outline-[red]"
              : "border-blue-600"
          }`}
        />
        {authTipsType !== "verification" && (
          <p className="mt-2 whitespace-nowrap text-xs text-[red]">
            {authTips}
          </p>
        )}
      </div>
      {type === "signup" && (
        <>
          ...
          {/* 错误提示语 */}
          {authTipsType === "verification" && (
            <p className="mt-2 ml-4 mb-4 whitespace-nowrap text-xs text-error-red">
              {authTips}
            </p>
          )}
        </>
