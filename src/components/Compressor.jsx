    ...
    listComponent = (
      <div className="w-full bg-white shadow-md rounded-md overflow-hidden">
        ...
        <div className="p-4 grid gap-6">
          {Array.from(compressorState.list.values())
            .reverse()
            .map((info) => {
              return (
                <div
                  key={info.key}
                  className="flex items-center justify-between pb-5 relative after:block after:absolute after:bottom-0 after:left-16 after:right-0 after:h-[1px] after:bg-slate-200"
                >
                  <div className="flex items-center max-w-[50%]">
                    <div className="overflow-hidden w-[48px] h-[48px] mr-4 rounded-md relative cursor-pointer [&_div]:hover:flex select-none">
                      <img
                        src={info.preview?.src || info.src}
                        className="w-full h-full flex-shrink-0 object-cover aspect-[1/1] relative z-0"
                      />
                      {info.compress?.src && (
                        <div
                    ...
                    <div className="flex-1 w-full">
                      <div className="font-semibold mb-1.5 truncate">
                        {info.name}
                      </div>
                      ...
                      <div className="px-1 text-right">
                        <div className="flex justify-end items-center gap-1">
                          <span className="font-bold text-sm">
                            {(
                              Math.abs(
                                (info.compress.blob.size - info.blob.size) /
                                  info.blob.size
                              ) * 100
                            ).toFixed(2)}
                            %
  ...
  return (
    <>
      ...
      <div className="bg-white border-b shadow-md">
        <div className="flex gap-4 p-2 justify-center flex-col-reverse md:flex-row md:justify-between">
          <div className="flex gap-3 items-center justify-center">
            ...
            >
              <Button disabled={disabled} icon={<Icon name="ImagePlus" />}>
                Add Images
              </Button>
            </Upload>
            ...
            ></Button>
            {showSetting && compressorState.list.size > 0 && (
              <span className="text-xs text-slate-500">
                Use Recompress to apply the new settings
              </span>
            )}
          </div>
      ...
      <div className="relative min-h-[200px] p-5">
        <div className="flex w-full justify-center z-10">{listComponent}</div>
      </div>
