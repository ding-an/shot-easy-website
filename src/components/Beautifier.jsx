...
export default function Beautifier() {
  ...
  const [waterCont, setWaterCont] = useState("ImgTools");
  ...
  useEffect(() => {
    if (ratioSize.key === "auto" && imgSize.width)
      setRatio(imgSize.width / imgSize.height);
  }, [imgSize, ratioSize]);
  ...
  const mergeCanvas = () =>
    new Promise((resolve, reject) => {
      ...
      toCanvas(boxRef.current, {
        ...
        canvasHeight:
          ratioSize.key === "auto" ? imgSize.height : ratioSize.height,
        ...
        .then((canvas) => {
          ...
          const annotate = canvasRef.current.toCanvas({
            pixelRatio:
              (ratioSize.key === "auto" ? imgSize.width : ratioSize.width) /
              boxWidth,
          });
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "polka flex flex-col rounded-md shadow-lg border-t overflow-hidden border-t-gray-600 antialiased",
          isFull ? "w-full h-full fixed z-10 top-0 left-0" : "min-h-[680px]"
        )}
      >
          ...
          <div className="flex gap-1 md:gap-4 justify-center items-center">
            <div className="px-0 text-xs opacity-80 md:px-2">
              {ratioSize.key === "auto"
                ...
                : `${ratioSize.width} x ${ratioSize.height}`}{" "}
              px
            </div>
            <DownBtn
              disabled={!photoUrl}
        ...
        <div className="flex flex-col flex-1 h-0 md:flex-row md:items-stretch">
          <div className="md:w-0 md:flex-1 overflow-auto select-none p-5">
            <div className="min-w-full md:min-w-[600px] min-h-full flex items-center justify-center">
              ...
              >
                {bgValue.includes("_img_") && (
                  <img
                    src={`${backgroundConfig[bgValue]}&w=1200`}
                ...
                >
                  <div className="absolute inset-0 flex items-center justify-center z-10">
                    ...
                    {photoUrl && !modelFrame.includes(frame) && (
                      <div
                        className={cn(
                        ...
                        {frame.includes("sBar") && (
                          <div
                            className={cn(
                              ...
                              frame === "macosBarLight" &&
                                "bg-[#f0f0f0] border-b border-b-[#e0e0e0]",
                              frame === "macosBarDark" &&
                                "bg-[#3a3a3c] border-b border-b-[#4a4a4c]",
                          ...
                          >
                            {frame.includes("macos") && (
                              <Icons.macos className="h-full" />
                            )}
                            {frame.includes("windows") && (
                              <Icons.windows className="h-full" />
                            )}
                          </div>
                        )}
                        <div
          ...
          <div className="bg-white flex flex-col gap-2 p-4 md:w-[340px] border-l border-l-gray-50 shadow-lg relative z-10">
            ...
            <div className="[&_label]:font-semibold [&_label]:text-sm">
              <div className="flex justify-between">
                ...
                <ColorPicker
                  value={paddingBg}
                  onChange={(e) => setPaddingBg(e.toRgbString())}
                  size="small"
                />
              </div>
              <Slider
            ...
            <div className="[&_label]:font-semibold [&_label]:text-sm">
              <div className="flex justify-between items-center">
                ...
                >
                  More
                  <Icon name="ChevronRight" style={{ marginLeft: 0 }} />
                </Button>
              </div>
              <div className="py-3">
                <Radio.Group
                  ...
                  >
                    <div
                      className={cn(
                        ...
                        backgroundConfig.default_1
                      )}
                    ></div>
                  </Radio>
                  {Object.keys(backgroundConfig).map((key, index) => {
                    if (key.includes("default") && key !== "default_1")
                      return (
                        <Radio
                          key={key}
                          className="[&_.ant-radio]:hidden [&_span]:p-0 mr-0"
            ...
            <div className="[&_label]:font-semibold [&_label]:text-sm flex justify-between items-center">
              ...
              {/* <Select
                            ...
                            /> */}
              <SelectDropdown
                defaultValue={ratioSize}
                onClick={onSelectDropdownClick}
              />
            </div>
            <div className="[&_label]:font-semibold [&_label]:text-sm flex justify-between items-center">
              <label>Frame</label>
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <div>
                  <Radio.Group
                    ...
                    size="small"
                  >
                    <Radio.Button value="contain" className="text-xs">
                      Contain
                    </Radio.Button>
                    <Radio.Button value="cover" className="text-xs">
                      Cover
                    </Radio.Button>
                    <Radio.Button value="fill" className="text-xs">
                      Fill
                    </Radio.Button>
                  </Radio.Group>
                </div>
            ...
            <div className="[&_label]:font-semibold [&_label]:text-sm flex gap-4 items-center">
              ...
              <Switch
                defaultChecked={useWater}
                onChange={(checked) => setUseWater(checked)}
                size="small"
            ...
            {useWater && (
              <div className="[&_label]:font-semibold [&_label]:text-xs grid gap-2 pl-4">
                <Input
                  defaultValue={waterCont}
                  placeholder="Watermark content"
                  onChange={(e) => setWaterCont(e.target.value)}
                />
                  ...
                  <div>
                    ...
                    >
                      <Radio.Button value={-22}>
                        <Icon name="ArrowUpRight" className="mt-[3px]" />
                      </Radio.Button>
                      <Radio.Button value={22}>
                        <Icon name="ArrowDownRight" className="mt-[3px]" />
                      </Radio.Button>
                    </Radio.Group>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    size="small"
                    onChange={(checked) => setWaterIndex(checked ? 9 : 11)}
                    className="bg-slate-200"
                  />
                </div>
              </div>
