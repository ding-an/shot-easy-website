...
const Toast = props => {
	...
	const parentOffset = position === ToastPosition.CENTER ? 'top-1/2 -translate-y-1/2' : 'top-0'
	const offset = position === ToastPosition.CENTER ? '' : 'mt-[80px]'

	return (
		<div className={`fixed left-0 right-0 -translate-x-0 ${parentOffset} ${offset} z-[1000]`}>
...
const alert = (content, position = ToastPosition.TOP_CENTER, duration = 3000) => {
	show(
		{
			content,
			position,
...
alert.success = (content, position = ToastPosition.TOP_CENTER, duration = 3000) => {
	show(
		{
			content,
			position,
...
alert.info = (content, position = ToastPosition.TOP_CENTER, duration = 3000) => {
	show(
		{
			content,
			position,
...
alert.tips = (content, position = ToastPosition.TOP_CENTER, duration = 3000) => {
	show(
		{
			content,
			position,
