...
export const validators = {
  notEmpty: (str) => (str && str.length > 0 ? 'validation_success' : 'required_field'),
  // next.js 有毒 invalid_input 找不到...
  reg: (str, reg) => (reg.test(str) ? 'validation_success' : 'invalid_ipt'),
  email: (str) =>
  ...
  option: (str, options) => {
    ...
    return options.includes(str) ? 'validation_success' : 'invalid_option'
  },
  cardNumber: (str) => {
