import nav from "./nav";
import editor from "./editor";
import beautifier from "./beautifier";
import rounded from "./rounded";
import remover from "./remover";
import compressor from "./compressor";
import screenshot from "./screenshot";

export default {
  title: "photo editor online for free",
  description:
    "Photo edit online for free, resize, and filter any photos, edit photo on browser, convert image to jpg/png/jpeg/webp, easy to screenshot area or full page",
  keywords:
    "ImgTools, easy shot, screenshot, edit photo, photo converter, image converter, editor online, change image format online, convert image to jpg, jpg to webp, jpg to png",
  privacy: "Privacy",
  terms: "Terms",
  blog: "Blog",
  nav,
  editor,
  beautifier,
  rounded,
  remover,
  compressor,
  screenshot,
};
